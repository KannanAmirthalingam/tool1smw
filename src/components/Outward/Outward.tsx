...
import { ArrowUpCircle, Plus, Search, User, Package } from 'lucide-react';
...
const Outward: React.FC = () => {
  ...
  const filteredOutwardEntries = outwardEntries.filter((entry: OutwardEntry) => {
    ...
    return matchesSearch && entry.status === 'issued';
  });
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-2xl font-bold text-purple-600">
                {filteredOutwardEntries.filter((entry: OutwardEntry) => {
            ...
            <ArrowUpCircle className="text-purple-600" size={24} />
          </div>
        </div>
      </div>
          ...
          <table className="w-full">
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {filteredOutwardEntries.map((entry: OutwardEntry) => {
                const daysIssued = Math.ceil((new Date().getTime() - new Date(entry.issued_date).getTime()) / (1000 * 60 * 60 * 24));
                const statusColor = daysIssued > 30 ? 'bg-red-100 text-red-800' : 
        ...
        {filteredOutwardEntries.length === 0 && (
          <div className="text-center py-12">
            ...
            <p className="text-gray-600">
              {searchTerm ? 'Try adjusting your search terms' : 'No tools have been issued yet'}
            </p>
          </div>
        )}
      </div>
